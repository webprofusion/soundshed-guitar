...
- **What changed goes to the message thread through a `NodeChangeQueue`** (`core/src/automation/NodeChangeQueue.h`), a fixed set of cells the audio thread claims by compare-exchange. The working copy of the preset and the UI's `signalPathNodeParamUpdated` are updated from it in `OnIdle`. `OnIdle` only runs while an editor is open, so a change replaces any not yet taken for the same node id and parameter, or for a bypass the same effect type: the queue holds the latest per target however long the editor stays closed, and the editor catches up with one message per target when it opens. The strings a change names (the node id, the binding) are shared pointers the audio thread only copies; `OnIdle` releases them.
