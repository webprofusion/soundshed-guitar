...
* Delay and reverb tails now carry over a preset or scene switch instead of being cut off. Everything you play goes to the new preset immediately; the old one only keeps ringing out what was already in its delay lines and reverb, and stops the moment it falls silent. Only presets that actually contain a delay or reverb ring on at all, and only one at a time. The limit is two bars at the current tempo, for a tail that would never stop on its own — set it to anything from Off to four bars under Settings → General → Preset Switching.
