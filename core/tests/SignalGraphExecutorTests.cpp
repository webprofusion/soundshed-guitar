...
void RegisterConfigProbeEffect()
{
    ...
    EffectRegistry::Instance().Register(info.type, info, []() { return std::make_unique<ConfigProbeEffect>(); });
}
...
bool TestNodeTypeConfigDefaultReachesCompositeInterior()
{
    ...
    EffectRegistry::Instance().Register(info.type, info,
                                        [def]() { return std::make_unique<CompositeEffectProcessor>(def); });
...
int main()
{
    ...
    // and +1.195 dB after it.)
    {
        ...
        auto runNam = [](const std::string& resourceId, bool useCalibration) {
            ...
            namRes.filePath =
                base / "amps" / "Guitar" / "A2" / "[AMP] MESA.MKVII-90W-CH1-CLN Factory Bright Clean - BLEND #1.nam";
            lib.AddResource(namRes);
        ...
        const bool ok = !uncalibrated.hasNaN && !calibrated.hasNaN && !uncalibrated.hasInf && !calibrated.hasInf &&
                        uncalibrated.rms > 1e-5 && calibrated.rms > 1e-6 && calibrated.rms < uncalibrated.rms * 0.8 &&
