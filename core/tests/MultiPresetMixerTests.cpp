...
static bool GraphHasNodeType(const SignalGraph& graph, const std::string& nodeType)
{
    return std::any_of(graph.nodes.begin(), graph.nodes.end(),
                       [&](const GraphNode& node) { return node.type == nodeType; });
}
...
int main()
{
        ...
        for (int i = 0; i < kTestBlockSize; ++i)
        {
                ...
                std::fabs(outMtR[static_cast<size_t>(i)] - outStR[static_cast<size_t>(i)]) > 1e-6f)
            {
                std::cerr << "Processing mode output mismatch at sample " << i
                          << ": MT(L=" << outMtL[static_cast<size_t>(i)] << ", R=" << outMtR[static_cast<size_t>(i)]
                          << ") ST(L=" << outStL[static_cast<size_t>(i)] << ", R=" << outStR[static_cast<size_t>(i)]
                          << ")" << std::endl;
                allPassed = false;
                break;
            }
    ...
    // Global user input calibration should apply a fixed gain before the chain
    {
        ...
        for (int i = 0; i < kTestBlockSize; ++i)
        {
            ...
            {
                std::cerr << "Global user input calibration mismatch at sample " << i << ": expected=" << expected
                          << " got L=" << outL[static_cast<size_t>(i)] << " R=" << outR[static_cast<size_t>(i)]
                          << std::endl;
                allPassed = false;
                break;
            }
    ...
    // Truncating leaves the caller's stale previous block in the tail, which clicks.
    {
        ...
        if (untouched > 0)
        {
            std::cerr << "Oversized block left " << untouched << "/" << oversized << " output samples stale"
                      << std::endl;
            allPassed = false;
        }
        ...
        {
            std::cerr << "Expected the oversized block to be counted once, got " << mixer.GetOversizedBlockCount()
                      << std::endl;
            allPassed = false;
        }
    }
