...
bool TestConcurrentWritesAndReads()
{
    ...
    ok &= Expect(stores[0]->Count(ItemType::kPreset) == kInstances * kWritesPerInstance,
                 "every write from every instance is present (" + std::to_string(stores[0]->Count(ItemType::kPreset)) +
                     " of " + std::to_string(kInstances * kWritesPerInstance) + ")");

    std::cout << "  " << kInstances << " instances x " << kWritesPerInstance << " writes: " << elapsedMs << " ms ("
              << (elapsedMs * 1000.0 / (kInstances * kWritesPerInstance)) << " us/write)\n";
...
bool TestConcurrentSettingsDoNotClobber()
{
    ...
    ok &= Expect(clobbered == 0, "each instance's own setting survived (" + std::to_string(clobbered) + " clobbered)");
...
bool TestSyncVersionBumpsAreNotLost()
{
    ...
    ok &= Expect(version == kInstances * kBumpsPerInstance, "no sync bump was lost (version " +
...
bool TestReplaceAllUnderContention()
{
    ...
    ok &= Expect(torn == 0,
                 "no reader observed a partially replaced collection (" + std::to_string(torn.load()) + " torn reads)");
...
bool TestNoCorruptionUnderSustainedContention()
{
    ...
    std::cout << "  [info] " << operations.load() << " mixed operations across " << kInstances << " instances over "
              << kDuration.count() << "s, database intact\n";
        ...
        workers.emplace_back([&, i]() {
            ...
            while (!stop)
            {
                stores[static_cast<std::size_t>(i)]->Put(
                    ItemType::kPreset, "keep-" + std::to_string(i) + "-" + std::to_string(n), {{"n", n}});
                ++n;
            }
        });
    }
