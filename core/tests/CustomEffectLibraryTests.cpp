...
bool TestRoundTrip()
{
    ...
    const bool ok = loadedEntry && loadedEntry->moduleResourceId == entry.moduleResourceId &&
                    loadedEntry->moduleResourceType == entry.moduleResourceType &&
                    loadedEntry->defaultParams == entry.defaultParams &&
                    loadedEntry->descriptorSummary.value("parameterCount", -1) == 2 && loadedEntry->tags.size() == 3;
