...
void ExpectPath(const std::filesystem::path& actual, const std::filesystem::path& expected, const std::string& what)
{
...
void TestResolvesPayloadToBundleRoot()
{
    ...
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.vst3/Contents/x86_64-linux/Foo.so"), "/plugins/Foo.vst3",
               "a Linux VST3 payload resolves to the bundle root");

    // LV2 anywhere: users pick manifest.ttl or the inner binary.
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.lv2/manifest.ttl"), "/plugins/Foo.lv2",
               "an LV2 manifest resolves to the bundle directory");

    // macOS Audio Unit: the executable has no extension at all.
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.component/Contents/MacOS/Foo"), "/plugins/Foo.component",
    ...
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.clap/Contents/x86_64-linux/Foo.so"), "/plugins/Foo.clap",
               "a CLAP bundle payload resolves to the bundle root");

    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.vst3"), "/plugins/Foo.vst3", "a bundle root resolves to itself");
}
...
void TestSameNameAncestorWins()
{
    ...
    // the same path.
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.vst3/Contents/x86_64-win/Foo.vst3"), "/plugins/Foo.vst3",
...
void TestContainerFolderDoesNotSwallowPlugin()
{
    ...
    ExpectPath(ResolvePluginBundlePath("/plugins/Container.vst3/Foo.vst3"), "/plugins/Container.vst3/Foo.vst3",
               "a differently named bundle ancestor does not swallow the plugin");

    ExpectPath(ResolvePluginBundlePath("/Audio.vst/Plugins/Foo.vst3"), "/Audio.vst/Plugins/Foo.vst3",
               "an unrelated suffixed folder high in the path is ignored");
}

void TestNonBundlePathsArePreserved()
{
    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.dll"), "/plugins/Foo.dll",
               "a bare VST2 DLL is left alone so the loader can report it");

    ExpectPath(ResolvePluginBundlePath("/plugins/somewhere/else.txt"), "/plugins/somewhere/else.txt",
               "an unrelated file is left alone");

    ExpectPath(ResolvePluginBundlePath(""), "", "an empty path stays empty");

    ExpectPath(ResolvePluginBundlePath("/plugins/Foo.vst3/"), "/plugins/Foo.vst3", "a trailing separator is trimmed");
}

void TestFormatDetection()
{
    ExpectFormat(PluginFormatFromPath("/plugins/Foo.vst3"), PluginFormat::VST3, "a .vst3 bundle is VST3");
    ExpectFormat(PluginFormatFromPath("/plugins/Foo.vst3/Contents/x86_64-linux/Foo.so"), PluginFormat::VST3,
                 "a payload is classified by its bundle, not its own extension");
    ExpectFormat(PluginFormatFromPath("/plugins/Foo.lv2/manifest.ttl"), PluginFormat::LV2, "an LV2 manifest is LV2");
    ...
    ExpectFormat(PluginFormatFromPath("/plugins/Foo.aaxplugin"), PluginFormat::AAX, "an .aaxplugin bundle is AAX");
