...
template <typename T> void PrintSamples(const std::string& label, const std::vector<T>& buffer, int count = 20)
{
...
{
  public:
    GraphHarness(const fs::path& resourcesDir, const fs::path& modelPath, const fs::path& irPath, double sampleRate,
                 int blockSize, const char* ampType = kAmpTypeStandard)
    {
    ...
    {
        mExecutor->SetNodeParam("amp", "drive", value);
    }

    void SetTone(double value)
    {
        mExecutor->SetNodeParam("amp", "tone", value);
    }

    void SetMix(double value)
    {
        mExecutor->SetNodeParam("cab", "mix", value);
    }
