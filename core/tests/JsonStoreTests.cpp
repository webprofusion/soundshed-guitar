...
bool TestReplaceAllIsScopedAndAtomic()
{
    ...
    replacement.push_back(
        {std::string(ItemType::kResource), "nam:new-3", MakeResource("new-3", "New Three").dump(), 0});
...
bool TestUnparseableDocumentIsSurvivable()
{
    ...
    ok &= Expect(store.GetRaw(ItemType::kDocument, "broken").value_or("") == "{not json",
                 "raw text is still retrievable");
...
bool TestDamagedDatabaseIsRefused()
{
    ...
    ok &= Expect(error.find("integrity") != std::string::npos, "the refusal says why: \"" + error + "\"");
    ok &= Expect(!damaged.IsOpen(), "the store stays closed");
    ok &=
        Expect(!damaged.Put(ItemType::kPreset, "new", {{"id", "new"}}), "nothing can be written into the damaged file");
    ...
    ok &= Expect(!rebuilt.GetMeta("schema_version").has_value(), "and unstamped, so the legacy import runs again");
    rebuilt.Close();
