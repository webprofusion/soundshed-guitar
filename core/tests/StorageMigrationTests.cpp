...
struct LegacyProfile
{
  ...
  explicit LegacyProfile(const fs::path& base)
  {
    ...
    WriteFile(root / "blends" / "library.json",
              R"([{"id":"blend-1","models":[]},{"id":"blend-2","models":[]}])");
...
bool TestImportsEverySource()
{
  ...
  ok &= Expect(setlists.has_value() && (*setlists)["cursorIndex"] == 2,
               "setlist envelope keeps its surrounding state");
...
{
  const auto base = fs::temp_directory_path()
    / ("soundshed-migration-untouched-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()));
  LegacyProfile profile(base);
