...
double tone)
{
    ...
    preset.graph.edges = {{input.id, amp.id, 0, 0, 1.0}, {amp.id, cab.id, 0, 0, 1.0}, {cab.id, output.id, 0, 0, 1.0}};
...
// Placeholder for future helper functions
{
  public:
    ...
    static bool Read(const std::string& filename, std::vector<std::vector<double>>& channels, double& sampleRate)
    {
        ...
        if (std::memcmp(header.riffId, "RIFF", 4) != 0 || std::memcmp(header.waveId, "WAVE", 4) != 0 ||
            std::memcmp(header.fmtId, "fmt ", 4) != 0 || std::memcmp(header.dataId, "data", 4) != 0)
        {
        ...
        std::cout << "Input WAV: " << numChannels << " channels, " << sampleRate << " Hz, " << bitsPerSample
                  << " bits, " << numSamples << " samples\n";
    ...
    static bool Write(const std::string& filename, const std::vector<std::vector<double>>& channels, double sampleRate,
                      int bitsPerSample = 16)
    {
        ...
        std::cout << "Output WAV written: " << numChannels << " channels, " << sampleRate << " Hz, " << bitsPerSample
                  << " bits, " << numSamples << " samples\n";
