...
#include "NAM/lstm.h"
                                     ...
                                     double tone)
                                     {
                                         ...
                                         preset.graph.edges = {{input.id, amp.id, 0, 0, 1.0},
                                       ...
                                       public:
                                                          ...
                                                          double& sampleRate)
                                         {
                                             ...
                                             const int numSamples =
                                                 header.dataSize / (numChannels * (bitsPerSample / 8));

                                             std::cout << "Input WAV: " << numChannels << " channels, " << sampleRate
                                                       << " Hz, " << bitsPerSample << " bits, " << numSamples
                                                       << " samples\n";
                                             ...
                                             else if (bitsPerSample == 24)
                                             {
                                                 for (int i = 0; i < numSamples; ++i)
                                                 {
                                                     for (int ch = 0; ch < numChannels; ++ch)
                                                     {
                                                         ...
                                                         int32_t sample =
                                                             (bytes[2] << 24) | (bytes[1] << 16) | (bytes[0] << 8);
                                     ...
                                     int main(int argc, char* argv[])
                                     {
                                         ...
                                         try
                                         {
                                             ...
                                             std::string inputFile =
                                                 (fs::path(resourcesDir) / "ui" / "demo" / "guitar-riff-02.wav")
                                                     .string();
                                             ...
                                             std::cout << (modelFile.empty() ? "No model specified, amp disabled"
                                                                             : "Model path: " + modelFile.string())
                                                       << "\n";
                                             std::cout << (irFile.empty() ? "No IR specified, cab disabled"
                                                                          : "IR path: " + irFile.string())
                                                       << "\n";
                                             ...
                                             while (processedSamples < numSamples)
                                             {
                                                 ...
                                                 {
                                                     inputBuffer[0][i] =
                                                         static_cast<float>(inputChannels[0][processedSamples + i]);
                                                     inputBuffer[1][i] =
                                                         static_cast<float>(inputChannels[1][processedSamples + i]);
                                                 }
                                                 ...
                                                 if (blockCount % 100 == 0)
                                                 {
                                                     ...
                                                     std::cout << "\rProcessing: " << static_cast<int>(progress)
                                                               << "%..." << std::flush;
                                                 }
                                             }
