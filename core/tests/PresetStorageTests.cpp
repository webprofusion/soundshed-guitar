...
bool TestSerializeDeserialize()
{
    ...
    // Verify metadata
    if (deserialized->id != original.id || deserialized->name != original.name ||
        deserialized->category != original.category || deserialized->description != original.description ||
    ...
    if (deserialized->global.inputTrim != 0.0 || deserialized->global.outputTrim != 0.0 ||
        deserialized->global.transpose != 0 || deserialized->globalSignalChain.has_value())
    {
    ...
    {
        std::cout << "FAIL (node count mismatch: " << deserialized->graph.nodes.size() << " vs "
                  << expected.graph.nodes.size() << ")" << std::endl;
        return false;
    }
...
bool TestNodeParams()
{
    ...
    // Check params
    if (deserializedNode.params.count("gain") == 0 || deserializedNode.params.at("gain") != 0.75)
    {
        std::cout << "FAIL (gain param mismatch)" << std::endl;
        return false;
    }

    if (deserializedNode.params.count("frequency") == 0 || deserializedNode.params.at("frequency") != 1000.0)
    {
    ...
    // Check config
    if (deserializedNode.config.count("mode") == 0 || deserializedNode.config.at("mode") != "stereo")
    {
...
bool TestPresetFilePathNormalizationOnDisk()
{
    ...
    else if (raw.contains("scenes") && raw["scenes"].is_array() && !raw["scenes"].empty() &&
             raw["scenes"][0].is_object())
    {
    ...
    if (loaded->embeddedResources.empty() ||
        loaded->embeddedResources[0].originalPath != inPresetFile.lexically_normal())
    {
