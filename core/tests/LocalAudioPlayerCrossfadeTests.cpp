/**
 * @file LocalAudioPlayerCrossfadeTests.cpp
 * @brief Focused tests for LocalAudioPlayerService's loop-wrap/seek crossfade
...
// Friend accessor granted by LocalAudioPlayerService — exposes just enough
...
struct LocalAudioPlayerServiceTestAccess
{
    static std::shared_ptr<LocalAudioPlayerService::TrackBuffer> MakeBuffer(
        std::vector<float> left, std::vector<float> right, double sampleRate)
    {
        auto buffer = std::make_shared<LocalAudioPlayerService::TrackBuffer>();
    ...
    static int ReadSourceWindow(LocalAudioPlayerService& svc,
                                const std::shared_ptr<LocalAudioPlayerService::TrackBuffer>& buffer,
...
using guitarfx::LocalAudioPlayerService;
using guitarfx::LocalAudioPlayerServiceTestAccess;

constexpr double kSampleRate = 48000.0;

// Minimal IPluginHost stub. The crossfade logic under test never touches the
// host; LocalAudioPlayerService's constructor just requires a reference.
...
std::unique_ptr<LocalAudioPlayerService> MakeService(guitarfx::IPluginHost& host, std::mutex& mutex)
{
    auto svc = std::make_unique<LocalAudioPlayerService>(
        host, mutex,
...
// Return type is deduced (rather than spelling LocalAudioPlayerService::
// TrackBuffer) because this function is not itself a friend of
// LocalAudioPlayerService — only LocalAudioPlayerServiceTestAccess is, so
...
auto MakeRampLoopBuffer(std::size_t total, std::size_t regionStart, std::size_t regionEnd)
{
    ...
    return LocalAudioPlayerServiceTestAccess::MakeBuffer(left, left, kSampleRate);
}

bool TestLoopWrapStaysInBoundsAndBlends()
{
    std::cout << "\n--- LocalAudioPlayerService Loop-Wrap Crossfade Tests ---\n";
    ...
    {
        const int written = LocalAudioPlayerServiceTestAccess::ReadSourceWindow(
...
{
    std::cout << "\n--- LocalAudioPlayerService Short-Loop Bounds Test ---\n";
...
{
    std::cout << "\n--- LocalAudioPlayerService Non-Looping Exhaustion Test ---\n";
    ...
    auto buffer = LocalAudioPlayerServiceTestAccess::MakeBuffer(left, left, kSampleRate);
    ...
    const int firstWritten = LocalAudioPlayerServiceTestAccess::ReadSourceWindow(
        *svc, buffer, outL.data(), outR.data(), cursor, kChunk);
    const bool firstCallShortAtEnd = firstWritten == 200 && cursor == kTotalFrames;

    const int secondWritten = LocalAudioPlayerServiceTestAccess::ReadSourceWindow(
        *svc, buffer, outL.data(), outR.data(), cursor, kChunk);
