...
const std::vector<double> kSampleRates = {22050.0, 32000.0, 44100.0, 48000.0, 88200.0, 96000.0};
...
guitarfx::Preset CreateResourceSyntheticPreset(const int index, const guitarfx::LibraryResource& nam,
...
void LoadLibraryResources(guitarfx::ResourceLibrary& library, const nlohmann::json& modelsJson,
                          const nlohmann::json& irJson, const fs::path& baseDir)
{
...
ScenarioData BuildScenario(const fs::path& resourcesDir, const guitarfx::ResourceLibrary& library, int maxPresets)
{
    ...
    for (const auto& presetJson : presetsJson)
    {
        ...
        scenario.presetIds.push_back(parsed->id.empty() ? ("preset-" + std::to_string(scenario.presets.size()))
                                                        : parsed->id);
...
template <typename T> T PickRandom(const std::vector<T>& values, std::mt19937_64& rng)
{
...
LONG WINAPI StressCrashHandler(EXCEPTION_POINTERS* info)
{
    ...
    std::cerr << "\nStress test crashed: code=0x" << std::hex << (record ? record->ExceptionCode : 0u) << " at address "
              << (record ? record->ExceptionAddress : nullptr) << std::dec
              << "\nSee the trace file for the seed and the last actions before the fault." << std::endl;
        ...
        for (int step = 0; step < cfg.steps; ++step)
        {
            ...
            if (mutationType < 15)
            {
                ...
                appendTrace("step=" + std::to_string(step) +
                            " action=prepare sampleRate=" + std::to_string(currentSampleRate));
            }
            else if (mutationType < 30)
            {
                ...
                appendTrace("step=" + std::to_string(step) +
                            " action=io mono=" + std::to_string(static_cast<int>(monoMode)) +
            ...
            else if (mutationType < 65)
            {
                ...
                appendTrace("step=" + std::to_string(step) + " action=presetState presetId=" + presetId +
                            " mix=" + std::to_string(mix) + " pan=" + std::to_string(pan) + " mute=" +
                            std::to_string(static_cast<int>(mute)) + " solo=" + std::to_string(static_cast<int>(solo)));
            }
            ...
            else if (mutationType < (resourceMode ? 82 : 96) && !scenario.paramTargets.empty())
            {
                ...
                appendTrace("step=" + std::to_string(step) + " action=nodeParam presetId=" + target.presetId +
                            " nodeId=" + target.nodeId + " type=" + target.nodeType + " param=" + target.paramId +
            ...
            else if (!scenario.resourceTargets.empty())
            {
                ...
                {
                    appendTrace(
                        "step=" + std::to_string(step) + " action=resourceRebindSkipped presetId=" + target.presetId +
