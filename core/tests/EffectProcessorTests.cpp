...
double EstimateFrequencyFromPositiveZeroCrossings(const std::vector<float>& buffer, double sampleRate)
{
...
std::vector<float> RenderDriveEffect(const std::string& effectType, double inputAmplitude, int blocksToProcess = 6)
{
    ...
    for (int block = 0; block < blocksToProcess; ++block)
    {
        ...
        {
            const double phase =
                2.0 * kPi * 220.0 * static_cast<double>(startIndex + static_cast<std::size_t>(i)) / kTestSampleRate;
...
bool TestEffectProcessor(const std::string& effectType)
{
    ...
    const int blocksToProcess = std::max((effectType == guitarfx::EffectGuids::kSynthSaw) ? 8 : 1, latencyBlocks + 2);
    ...
    try
    {
        for (int block = 0; block < blocksToProcess; ++block)
        {
            ...
            float* blkInputs[2] = {inputL.data() + block * kTestBlockSize, inputR.data() + block * kTestBlockSize};
    ...
    std::cout << "  PASS (peak=" << std::fixed << std::setprecision(3) << outputAnalysis.peakValue
...
bool TestAutoArpSpecific()
{
    ...
    // Test 1: SetParam round-trip for timing params
    {
        ...
        const bool ok =
            (e->GetParam("bpm") == 130.0) && (e->GetParam("stepRate") == 2.0) && (e->GetParam("numSteps") == 3.0);
...
bool TestFlangerReverbStability()
{
    ...
    try
    {
        ...
        for (int block = 0; block < totalBlocks; ++block)
        {
            ...
            {
                const double phase =
                    2.0 * kPi * 110.0 * static_cast<double>(startIndex + static_cast<std::size_t>(i)) / kTestSampleRate;
        ...
        std::cout << "  Peak=" << std::fixed << std::setprecision(3) << peak << ", RMS=" << rms << " -> "
                  << (ok ? "PASS" : "FAIL") << "\n";
        return ok;
    }
    catch (const std::exception& ex)
...
bool TestAlgorithmicReverbsStaySilentOnSilence()
{
    ...
    const std::array<const char*, 5> reverbTypes = {
        guitarfx::EffectGuids::kReverbRoom, guitarfx::EffectGuids::kReverbChamber, guitarfx::EffectGuids::kReverbSpring,
        guitarfx::EffectGuids::kReverbAdvanced, guitarfx::EffectGuids::kReverbAmbient};
...
bool TestSpringReverbTailDecaysCleanly()
{
    ...
    std::cout << "  Spring tail decay: " << (decayed ? "PASS" : "FAIL") << " (tailPeak=" << std::scientific << tailPeak
              << ")\n"
              << std::defaultfloat;
    return decayed;
}
...
bool TestTempoSyncSpecific()
{
    ...
    const std::vector<TempoCase> cases = {
        {"Delay quarter-note at 120 BPM", guitarfx::EffectGuids::kDelayDigital, 120.0, 4.0, "effectiveTimeMs", 500.0,
         0.01},
        ...
        {"Phaser eighth-note triplet at 120 BPM", guitarfx::EffectGuids::kPhaser, 120.0, 9.0, "effectiveRate", 6.0,
         0.01},
    };
    ...
    for (const auto& testCase : cases)
    {
        ...
        std::cout << "  " << std::left << std::setw(44) << testCase.label << (ok ? "PASS" : "FAIL")
...
bool TestDriveEffectCharacter()
{
    ...
    std::cout << "  Distinct transfer/output shapes:             " << (distinctOk ? "PASS" : "FAIL")
              << " (OD/DIST=" << std::fixed << std::setprecision(3) << odVsDist << ", OD/FUZZ=" << odVsFuzz
...
bool TestGraphicEQProfilesAndStability()
{
    ...
    const bool validFrequencyRange = effect->GetParam("band1Freq") >= 20.0 && effect->GetParam("band2Freq") >= 20.0;
    ...
    if (typeInfo)
    {
        ...
        profilesAvailable = presetParam != typeInfo->parameters.end() && presetParam->labels.size() == 6 &&
                            presetParam->labels[0] == "Bass · 5 Band" &&
                            presetParam->labels[5] == "General Purpose · 10 Band" && typeInfo->presets.size() == 6;
        ...
        profileConfigApplied = applyPreset("bass-10") && effect->GetParam("bandCount") == 10.0 &&
                               effect->GetParam("band1Freq") == 45.0 && effect->GetParam("band1Q") == 2.0;
        ...
        guitarProfileApplied = applyPreset("guitar-10") && effect->GetParam("bandCount") == 10.0 &&
                               effect->GetParam("band8Freq") == 3000.0 && effect->GetParam("band8Q") == 0.9 &&
                               allBandsFlat();
    ...
    std::cout << "  Profile selection does not overwrite edits:   "
              << (profileSelectionIsMetadataOnly ? "PASS" : "FAIL") << "\n";
    ...
    return clampedBandCount && clampedFrequency && validFrequencyRange && profileConfigApplied &&
           guitarProfileApplied && profileSelectionIsMetadataOnly && profilesAvailable && finite;
}
...
bool TestDynamicsSoftClipOptions()
{
    ...
    const auto limiterHard =
        renderEffect(guitarfx::EffectGuids::kLimiterBrickwall, 1.2, [](guitarfx::EffectProcessor& effect) {
            effect.SetParam("ceiling", -6.0);
            effect.SetParam("release", 50.0);
        });
    const auto limiterSoft =
        renderEffect(guitarfx::EffectGuids::kLimiterBrickwall, 1.2, [](guitarfx::EffectProcessor& effect) {
            effect.SetParam("ceiling", -6.0);
            effect.SetParam("release", 50.0);
    ...
    const auto vcaHard =
        renderEffect(guitarfx::EffectGuids::kCompressorVca, 0.35, [](guitarfx::EffectProcessor& effect) {
    ...
    const auto optoHard =
        renderEffect(guitarfx::EffectGuids::kCompressorOpto, 0.35, [](guitarfx::EffectProcessor& effect) {
            effect.SetParam("threshold", 0.0);
            effect.SetParam("ratio", 1.0);
    ...
    if (limiterHard.empty() || limiterSoft.empty() || vcaHard.empty() || vcaSoft.empty() || optoHard.empty() ||
        optoSoft.empty())
    {
    ...
    const bool vcaSoftProtects =
        vcaHardMetrics.peak > 1.1 && vcaSoftMetrics.peak <= 1.01 && NormalizedDifference(vcaHard, vcaSoft) > 0.05;
    const bool optoSoftProtects =
        optoHardMetrics.peak > 1.1 && optoSoftMetrics.peak <= 1.01 && NormalizedDifference(optoHard, optoSoft) > 0.05;
    ...
    std::cout << "  VCA soft clip protects post-makeup output:   " << (vcaSoftProtects ? "PASS" : "FAIL")
              << " (hard peak=" << vcaHardMetrics.peak << ", soft peak=" << vcaSoftMetrics.peak << ")\n";
    std::cout << "  Opto soft clip protects post-makeup output:  " << (optoSoftProtects ? "PASS" : "FAIL")
              << " (hard peak=" << optoHardMetrics.peak << ", soft peak=" << optoSoftMetrics.peak << ")\n";
...
bool TestNamWetDryMixSpecific()
{
    ...
    for (const auto& testCase : cases)
    {
        ...
        const auto mixIt = infoOpt ? std::find_if(infoOpt->parameters.begin(), infoOpt->parameters.end(),
                                                  [](const guitarfx::ParameterDef& param) { return param.id == "mix"; })
    ...
    const bool legacyDryOk =
        runSingleModelDryTest(guitarfx::EffectGuids::kAmpNam, "Legacy NAM dry mix preserves input:");
    const bool optimizedDryOk =
        runSingleModelDryTest(guitarfx::EffectGuids::kAmpNamOptimized, "Optimized NAM dry mix preserves input:");
...
bool TestOctaveSpecific()
{
    ...
    auto runVoiceTest = [&](double octaveUp, double octaveDown, const char* label) {
        ...
        for (int block = 0; block < kBlocksToProcess; ++block)
        {
            float* inputs[2] = {inputL.data() + block * kTestBlockSize, inputR.data() + block * kTestBlockSize};
            effect->Process(inputs, outputs, kTestBlockSize);
        }
        ...
        std::cout << "  " << std::left << std::setw(44) << label << (ok ? "PASS" : "FAIL")
                  << " (latency=" << effect->GetLatencySamples() << ", peak=" << std::fixed << std::setprecision(3)
                  << analysis.peakValue << ")\n";
        return ok;
    };
...
bool TestTransposeLatencySpecific()
{
    ...
    std::cout << "  " << std::left << std::setw(44)
              << "0 semitones reports transparent latency:" << (zeroShiftTransparent ? "PASS" : "FAIL")
              << " (latency=" << zeroShiftLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "-1 semitone reports full (in+out) latency:" << (lowShiftReportsLatency ? "PASS" : "FAIL")
              << " (latency=" << lowShiftLatency << ", min=" << expectedMinTotalLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "-2 semitones keeps current latency profile:" << (slightDownTuneKeepsLatencyProfile ? "PASS" : "FAIL")
              << " (latency=" << slightDownTuneLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "-12 semitones keeps current latency profile:" << (deepShiftKeepsLatencyProfile ? "PASS" : "FAIL")
...
bool TestHybridTransposeSpecific()
{
    ...
    auto renderEffect = [&](guitarfx::EffectProcessor& effect, const std::vector<float>& input,
        ...
        for (int block = 0; block < totalSamples / kTestBlockSize; ++block)
        {
            ...
            float* inputs[2] = {const_cast<float*>(input.data()) + block * kTestBlockSize,
    ...
    const size_t warmupStart = static_cast<size_t>(std::min(totalSamples - 1, deepLatency + kTestBlockSize * 2));
    ...
    const size_t onsetStart =
        static_cast<size_t>(std::min(transientSamples - 1, (latencyBlocks + 2) * kTestBlockSize + deepLatency));
    const size_t onsetEnd = std::min(assistedOutput.size(), onsetStart + static_cast<size_t>(kTestBlockSize));
    const size_t steadyStart =
        std::min(assistedOutput.size() - 1, onsetStart + static_cast<size_t>(kTestBlockSize * 3));
    ...
    const bool mediumShiftSelectsQualityMode =
        lightLatency > 0 && mediumLatency == expectedMediumLatency && mediumLatency > lightLatency;
    const bool deepShiftSelectsQualityMode = deepLatency == expectedDeepLatency && deepLatency >= mediumLatency;
    const bool sineOutputHealthy =
        !sineAnalysis.hasNaN && !sineAnalysis.hasInf && !sineAnalysis.isAllZeros && sineAnalysis.peakValue > 0.05;
    const bool octaveDownDetected = detectedFrequency >= 200.0 && detectedFrequency <= 240.0;
    const bool chordOutputHealthy =
        !chordAnalysis.hasNaN && !chordAnalysis.hasInf && !chordAnalysis.isAllZeros && chordAnalysis.peakValue > 0.05;
    const bool transientOnsetStable =
        onsetPeak > 0.03 && steadyPeak > 0.03 && onsetPeak < steadyPeak * 2.5 && onsetPeak > steadyPeak * 0.35;

    std::cout << "  " << std::left << std::setw(44)
              << "0 st bypass clears reported latency:" << (bypassLatencyClears ? "PASS" : "FAIL")
              << " (latency=" << bypassLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Medium shifts use quality latency path:" << (mediumShiftSelectsQualityMode ? "PASS" : "FAIL")
              << " (-3=" << lightLatency << ", -4=" << mediumLatency << ", expected=" << expectedMediumLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Deep shifts stay on quality latency path:" << (deepShiftSelectsQualityMode ? "PASS" : "FAIL")
              << " (-12=" << deepLatency << ", expected=" << expectedDeepLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Steady-state sine output remains valid:" << (sineOutputHealthy ? "PASS" : "FAIL")
              << " (peak=" << std::fixed << std::setprecision(3) << sineAnalysis.peakValue << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "-12 st lands near one octave down:" << (octaveDownDetected ? "PASS" : "FAIL")
              << " (freq=" << std::fixed << std::setprecision(1) << detectedFrequency << " Hz)\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Triad input remains finite and audible:" << (chordOutputHealthy ? "PASS" : "FAIL")
              << " (peak=" << std::fixed << std::setprecision(3) << chordAnalysis.peakValue << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Bright transient stays audible and bounded:" << (transientOnsetStable ? "PASS" : "FAIL")
              << " (onsetPeak=" << std::fixed << std::setprecision(3) << onsetPeak << ", steadyPeak=" << steadyPeak
              << ")\n";

    return bypassLatencyClears && deepShiftSelectsQualityMode && mediumShiftSelectsQualityMode && sineOutputHealthy &&
           octaveDownDetected && chordOutputHealthy && transientOnsetStable;
}
...
bool TestHybridTransposeLiveChangesSpecific()
{
    ...
    for (const int semitones : semitoneSequence)
    {
        ...
        const int expectedLatency = (semitones == 0)
                                        ? 0
                                        : guitarfx::detail::StftTransposeChannel::GetExpectedLatencySamples(
                                              semitones, guitarfx::detail::GetHybridTransposePitchMode(semitones));
        ...
        for (int block = 0; block < 4; ++block)
        {
            ...
            const auto leftAnalysis = AnalyzeSignal(outputL);
            const auto rightAnalysis = AnalyzeSignal(outputR);
            allBlocksHealthy = allBlocksHealthy && !leftAnalysis.hasNaN && !leftAnalysis.hasInf &&
                               !rightAnalysis.hasNaN && !rightAnalysis.hasInf;
            allBlocksBounded = allBlocksBounded && leftAnalysis.peakValue < 4.0 && rightAnalysis.peakValue < 4.0;
        }
    }

    std::cout << "  " << std::left << std::setw(44)
              << "Reported latency tracks requested target:" << (latencyTracksTarget ? "PASS" : "FAIL") << "\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Rapid semitone changes stay finite:" << (allBlocksHealthy ? "PASS" : "FAIL") << "\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Rapid semitone changes stay bounded:" << (allBlocksBounded ? "PASS" : "FAIL") << "\n";
...
bool TestStftTransposeSpecific()
{
    ...
    const size_t warmupStart = static_cast<size_t>(std::min(totalSamples - 1, latencySamples + kTestBlockSize * 2));
    ...
    const bool outputHealthy =
        !analysis.hasNaN && !analysis.hasInf && !analysis.isAllZeros && analysis.peakValue > 0.05;
    const bool octaveDownDetected = detectedFrequency >= 205.0 && detectedFrequency <= 235.0;

    std::cout << "  " << std::left << std::setw(44)
              << "Latency stays within two blocks:" << (latencyBounded ? "PASS" : "FAIL")
              << " (latency=" << latencySamples << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Steady-state output remains valid:" << (outputHealthy ? "PASS" : "FAIL") << " (peak=" << std::fixed
              << std::setprecision(3) << analysis.peakValue << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "-12 st lands near one octave down:" << (octaveDownDetected ? "PASS" : "FAIL")
              << " (freq=" << std::fixed << std::setprecision(1) << detectedFrequency << " Hz)\n";
...
bool TestStftTransposeLiveChangesSpecific()
{
    ...
    std::cout << "  " << std::left << std::setw(44)
              << "Reported latency tracks pending target:" << (latencyTracksTarget ? "PASS" : "FAIL") << "\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Rapid semitone changes stay finite:" << (allBlocksHealthy ? "PASS" : "FAIL") << "\n";
...
bool TestStftTransposePolyphonicModeSpecific()
{
    ...
    const bool polyphonicOutputHealthy =
        !analysis.hasNaN && !analysis.hasInf && !analysis.isAllZeros && analysis.peakValue > 0.05;

    std::cout << "  " << std::left << std::setw(44)
              << "Polyphonic mode increases analysis window:" << (polyModeReportsHigherLatency ? "PASS" : "FAIL")
              << " (latency=" << latencySamples << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Chord input remains finite and audible:" << (polyphonicOutputHealthy ? "PASS" : "FAIL")
              << " (peak=" << std::fixed << std::setprecision(3) << analysis.peakValue << ")\n";
...
bool TestPitchShiftLatencySpecific()
{
    ...
    std::cout << "  " << std::left << std::setw(44)
              << "0 st reports transparent latency:" << (zeroShiftTransparent ? "PASS" : "FAIL")
              << " (latency=" << zeroShiftLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Pitch shift reports full (in+out) latency:" << (reportsPositiveLatency ? "PASS" : "FAIL")
              << " (latency=" << initialLatency << ", min=" << expectedMinTotalLatency << ")\n";
    std::cout << "  " << std::left << std::setw(44) << "Latency stays stable across pitch changes:"
              << (latencyRemainsStableAcrossPitchChanges ? "PASS" : "FAIL") << " (latency=" << updatedLatency << ")\n";
    ...
    const bool transposeApplied =
        wetCrossings > expectedDry * 1.4 && wetCrossings > expectedWet * 0.6 && wetCrossings < expectedWet * 1.4;
    const bool paramRetained = std::abs(preload->GetParam("semitones") - 12.0) < 1.0e-9;

    std::cout << "  " << std::left << std::setw(44)
              << "Preload +12 st retained after Prepare:" << (paramRetained ? "PASS" : "FAIL")
              << " (semitones=" << preload->GetParam("semitones") << ")\n";
    std::cout << "  " << std::left << std::setw(44)
              << "Preload +12 st shifts pitch (ZCR):" << (transposeApplied ? "PASS" : "FAIL")
              << " (crossings=" << wetCrossings << ", expected~" << static_cast<int>(expectedWet) << ", dry~"
              << static_cast<int>(expectedDry) << ")\n";
