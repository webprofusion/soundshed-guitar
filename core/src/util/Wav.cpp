...
std::optional<DecodedWav> DecodePcmWav(const std::vector<std::uint8_t>& bytes)
{
    ...
    if (audioFormat == 0 || channels == 0 || sampleRate == 0 || bitsPerSample == 0 || blockAlign == 0 ||
        dataOffset == 0)
    {
        return std::nullopt;
    }
    ...
    for (std::size_t frame = 0; frame < frameCount; ++frame)
    {
        ...
        for (std::size_t ch = 0; ch < static_cast<std::size_t>(channels); ++ch)
        {
            ...
            if (isFloat)
            {
                ...
                {
                    float v;
                    std::memcpy(&v, bytes.data() + so, 4);
                    sample = v;
                }
                else if (bitsPerSample == 64)
                {
                    std::memcpy(&sample, bytes.data() + so, 8);
                }
                else
                {
                    return std::nullopt;
            ...
            else
            {
                ...
                {
                case 8:
                    sample = (static_cast<double>(bytes[so]) - 128.0) / 128.0;
                    break;
                case 16:
                    sample = static_cast<double>(static_cast<std::int16_t>(ReadUint16LE(bytes.data() + so))) / 32768.0;
                    break;
                case 24: {
                ...
                case 32:
                    sample =
                        static_cast<double>(static_cast<std::int32_t>(ReadUint32LE(bytes.data() + so))) / 2147483648.0;
                    break;
                default:
                    return std::nullopt;
                }
                                                    ...
                                                    guitarfx::SampleRateConversionQuality quality)
{
    ...
    resampler.Prepare(sourceRate, targetRate, static_cast<int>(sourceFrames), quality);
...
bool WriteStereo16BitWav(const std::filesystem::path& path, const std::vector<float>& left,
