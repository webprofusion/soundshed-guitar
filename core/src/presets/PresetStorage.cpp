...
nlohmann::json SerializeGraphNode(const GraphNode& node, const std::optional<std::filesystem::path>& baseDirectory)
{
    nlohmann::json json;
    json["id"] = node.id;
    json["type"] = node.type;
...
GraphNode DeserializeGraphNode(const nlohmann::json& json, const std::optional<std::filesystem::path>& baseDirectory)
{
    GraphNode node;
                                             ...
                                             const std::optional<std::filesystem::path>& baseDirectory)
{
    ...
    res.originalPath = ResolveStoredPathForRuntime(util::PathFromUtf8(json.value("originalPath", "")), baseDirectory);
    return res;
}

nlohmann::json SerializePresetScene(const PresetScene& scene, const std::optional<std::filesystem::path>& baseDirectory)
{
    nlohmann::json json;
    json["id"] = scene.id;
