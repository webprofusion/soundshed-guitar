...
namespace guitarfx
{
  ...
  public:
    ...
    void SetParam(const std::string &key, double value) override
    {
      if (key == "rate")
      {
        mRateHz = static_cast<float>(std::clamp(value, 0.05, 5.0));
      }
      else if (key == "depth")
      {
        mDepthMs = static_cast<float>(std::clamp(value, 0.0, 5.0));
      }
      else if (key == "delay")
      {
        mDelayMs = static_cast<float>(std::clamp(value, 0.1, 5.0));
      }
