  ...
  private:
    ...
    bool LoadModelResource(const std::filesystem::path& resourcePath, const ResourceRef* ref)
    {
        try
        {
            ...
            mModelInputLevel =
                mModelLeft->HasInputLevel() ? std::optional<double>(mModelLeft->GetInputLevel()) : std::nullopt;
            mModelOutputLevel =
                mModelLeft->HasOutputLevel() ? std::optional<double>(mModelLeft->GetOutputLevel()) : std::nullopt;
    ...
    {
        return NearlyEqual(mBassDb, 0.0) && NearlyEqual(mMidDb, 0.0) && NearlyEqual(mTrebleDb, 0.0) &&
    ...
    void ConfigureModelProcessing()
    {
        ...
        mMaxModelBlockSize = std::max(1, static_cast<int>(std::ceil(static_cast<double>(mMaxBlockSize) *
                                                                    mModelSampleRate / std::max(mSampleRate, 1.0))) +
                                             1);
    ...
    void ResetModelsIfNeeded()
    {
        ...
        if (!mModelsNeedReset && mResetModelSampleRate == mModelSampleRate &&
        ...
        if (mModelLeft && mModelRight)
        {
            auto right = std::async(std::launch::async, [this]() {
                mOversamplingRight.Prepare(*mModelRight, mSampleRate, mBaseModelSampleRate, mMaxBlockSize,
                                           NamOversamplingFactorFromIndex(mOversamplingIndex),
                                           NamAntiAliasPhaseFromIndex(mAntiAliasPhaseIndex));
            });
            mOversamplingLeft.Prepare(*mModelLeft, mSampleRate, mBaseModelSampleRate, mMaxBlockSize,
                                      NamOversamplingFactorFromIndex(mOversamplingIndex),
                                      NamAntiAliasPhaseFromIndex(mAntiAliasPhaseIndex));
            right.get();
        }
    ...
    void ProcessModels(int numSamples)
    {
        ...
        if (rtparallel::ShouldParallelizeStereoWork(numSamples))
        {
            ranParallel = rtparallel::DualLaneExecutor::Instance().Run(
                [&]() {
                    mOversamplingRight.Process(*mModelRight, mInputBufferR.data(), mOutputBufferR.data(), numSamples);
                },
                [&]() {
...
inline void RegisterOptimizedNAMAmpEffect()
{
    ...
    info.parameters = {{"inputGain", "Input", 0.0, -24.0, 24.0, "dB", "Level"},
