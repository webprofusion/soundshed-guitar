...
class ReverbEffect : public EffectProcessor
{
  public:
    ...
    explicit ReverbEffect(Mode mode) : mMode(mode)
    {
        ...
        for (int i = 0; i < numSamples; ++i)
        {
            ...
            for (size_t c = 0; c < kCombCount; ++c)
            {
                ...
                const float dL = std::clamp(mBaseCombSamplesL[c] * mSizeScaleCurrent * combScaleL, 1.0f,
                                            static_cast<float>(mCombBufferL[c].size() - 2));
                const float dR = std::clamp(mBaseCombSamplesR[c] * mSizeScaleCurrent * combScaleR, 1.0f,
                                            static_cast<float>(mCombBufferR[c].size() - 2));
            ...
            for (size_t a = 0; a < kAllpassCount; ++a)
            {
                ...
                const float dL = std::clamp(mBaseAllpassSamplesL[a] * mSizeScaleCurrent * allpassScaleL, 1.0f,
                                            static_cast<float>(mAllpassBufferL[a].size() - 2));
                const float dR = std::clamp(mBaseAllpassSamplesR[a] * mSizeScaleCurrent * allpassScaleR, 1.0f,
                                            static_cast<float>(mAllpassBufferR[a].size() - 2));
  ...
  private:
    ...
    void ComputeBandpass(double freq, double q, double& b0, double& b1, double& b2, double& a1, double& a2) const
    {
        if (mSampleRate <= 0.0)
        {
            b0 = 1.0;
    ...
    void UpdateParameters()
    {
        ...
        for (size_t t = 0; t < kEarlyTapCount; ++t)
        {
            ...
            mEarlyTapSamplesMirror[t] =
                DelayMsToSamples((profile.earlyTapMs[t] + 0.9 + static_cast<double>(t) * 0.7) * (0.9 + 0.4 * mSize));
            mEarlyTapSamples[t] = std::min(mEarlyTapSamples[t], mPreDelayL.empty() ? size_t(1) : mPreDelayL.size() - 1);
            mEarlyTapSamplesMirror[t] =
                std::min(mEarlyTapSamplesMirror[t], mPreDelayL.empty() ? size_t(1) : mPreDelayL.size() - 1);
        ...
        if (mMode == Mode::Spring || (mMode == Mode::Advanced && mCharacter == Mode::Spring))
        {
            ...
            ComputeBandpass(centerHz, q, mSpringBpB0, mSpringBpB1, mSpringBpB2, mSpringBpA1, mSpringBpA2);
            ...
            ComputeBandpass(centerHz2, q2, mSpringBp2B0, mSpringBp2B1, mSpringBp2B2, mSpringBp2A1, mSpringBp2A2);
        }
    }
...
{
    const std::array<ReverbEffect::Mode, 3> modes = {ReverbEffect::Mode::Room, ReverbEffect::Mode::Chamber,
    ...
    for (const auto mode : modes)
    {
        ...
        case ReverbEffect::Mode::Advanced:
            ...
            info.parameters = {ParameterDef{"character",
                                            "Character",
                                            0.0,
                                            0.0,
                                            2.0,
