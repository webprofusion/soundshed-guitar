...
void CompositeEffectLibrary::AddDefinition(const CompositeEffectDefinition& def)
{
    ...
    auto it = std::find_if(mDefinitions.begin(), mDefinitions.end(), [&](const auto& d) { return d.id == def.id; });
...
void CompositeEffectLibrary::RemoveDefinition(const std::string& id)
{
    ...
    mDefinitions.erase(
        std::remove_if(mDefinitions.begin(), mDefinitions.end(), [&](const auto& d) { return d.id == id; }),
        mDefinitions.end());
}
...
bool CompositeEffectLibrary::SaveDefinition(const CompositeEffectDefinition& def, const std::filesystem::path& userDir)
{
    if (!def.IsValid())
    {
        return false;
