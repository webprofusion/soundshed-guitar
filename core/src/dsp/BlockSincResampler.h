...
class BlockSincResampler
{
  public:
    ...
    void Prepare(double sourceRate, double targetRate, int maxInputFrames,
    ...
    [[nodiscard]] int GetMaxOutputFrames() const noexcept
    {
    ...
    int ProcessFixedOutput(const InputSample* input, int inputFrames, OutputSample* output, int outputFrames) const
    {
  ...
  private:
    ...
    void EnsureKernelTable(int halfTaps, double cutoff) const
    {
        ...
        if (mKernelHalfTaps == halfTaps && std::abs(mKernelCutoff - cutoff) <= std::numeric_limits<double>::epsilon())
        {
            return;
        }
        ...
        for (int phaseIndex = 0; phaseIndex < kKernelPhaseCount; ++phaseIndex)
        {
            ...
            for (int tapOffset = -halfTaps; tapOffset <= halfTaps; ++tapOffset)
            {
                ...
                const std::size_t tableIndex =
                    static_cast<std::size_t>(phaseIndex) * static_cast<std::size_t>(tapCount) +
    ...
    template <typename InputSample, typename OutputSample>
    int ProcessSinc(const InputSample* input, int inputFrames, OutputSample* output, int outputFrames) const
    {
        const int halfTaps = GetHalfTapsForQuality();
        const int tapCount = 2 * halfTaps + 1;
        const double sourceStep = static_cast<double>(inputFrames) / static_cast<double>(outputFrames);
