...
template <typename Func> BenchmarkResult RunBenchmark(const std::string& name, Func&& func, long long iterations)
{
    // Warmup
    for (int i = 0; i < 10; ++i)
    {
...
{
    std::cout << std::left << std::setw(40) << result.name << std::right << std::setw(10) << std::fixed
              << std::setprecision(3) << result.durationMs << " ms"
...
inline void BenchmarkActivations()
{
    ...
    case simd::SimdLevel::AVX2:
        std::cout << "AVX2";
        break;
    case simd::SimdLevel::AVX:
        std::cout << "AVX";
        break;
    case simd::SimdLevel::SSE4:
        std::cout << "SSE4";
        break;
    case simd::SimdLevel::SSE2:
        std::cout << "SSE2";
        break;
    default:
        std::cout << "Scalar";
    ...
    // Fast Scalar Tanh
    auto fastScalarTanh = RunBenchmark(
        "Fast Scalar Tanh",
        [&]() {
            data = dataBackup;
    ...
    // Scalar Sigmoid
    auto scalarSigmoid = RunBenchmark(
        "Scalar Sigmoid",
        [&]() {
            data = dataBackup;
            for (long i = 0; i < bufferSize; ++i)
    ...
    auto simdGated = RunBenchmark(
        "SIMD Fused Gated (auto)",
        [&]() {
            data = dataBackup;
            gate = gateBackup;
...
inline void BenchmarkMatrixOps()
{
    ...
    auto simdOptimized = RunBenchmark(
