...
{
    return folderId.rfind("factory-archive::", 0) == 0 || folderId.rfind("factory-archive-folder::", 0) == 0;
}
...
nlohmann::json BuildFactoryArchiveFolders(const std::string& archiveKey, const nlohmann::json& archivePresetFolders,
    ...
    buildFolders = [&](const nlohmann::json& sourceFolders, const std::string& parentPath) -> nlohmann::json {
        ...
        for (const auto& sourceFolder : sourceFolders)
        {
            ...
            nlohmann::json folder =
                MakePresetFolderEntry(BuildFactoryArchiveNestedFolderId(archiveKey, folderPath), name);
...
void UpdateFactoryPresetFolders(guitarfx::storage::JsonStore& store, const std::string& archiveKey,
                                ...
                                const std::vector<std::string>&)
{
    ...
    auto archiveFolders = BuildFactoryArchiveFolders(archiveKey, archivePresetFolders, presetIdMapping);
                                                ...
                                                const std::unordered_map<std::string, std::string>& presetIdMapping)
{
    ...
    buildFolders = [&](const nlohmann::json& sourceFolders, const std::string& parentPath) -> nlohmann::json {
        ...
        for (const auto& sourceFolder : sourceFolders)
        {
            ...
            const std::string folderPath = parentPath.empty() ? name : (parentPath + "/" + name);
            nlohmann::json folder =
                MakePresetFolderEntry(BuildPresetArchiveSessionFolderId(archiveKey, folderPath), name);
