...
namespace guitarfx::storage
{
  namespace
  {
    ...
    void WriteLegacyTreeMarker(const std::filesystem::path& settingsDirectory,
                               ...
                               std::int64_t itemsImported)
    {
      try
      {
        ...
        marker << "The JSON files in this folder were imported into a database on "
               << NowMillis() << " (unix ms).\n\n"
    ...
    struct Importer
    {
      ...
      void ImportArray(const std::filesystem::path& path,
                       const char* itemType,
                       const char* idField,
      ...
      void ImportDirectory(const std::filesystem::path& directory,
                           const char* itemType,
                           const std::string& sourceLabel,
    ...
    void RunImport(Importer& importer,
                   ...
                   const std::filesystem::path& userPresetDirectory)
    {
      ...
      importer.ImportDirectory(settingsDirectory / "composite-presets", ItemType::kCompositePreset,
                                        ...
                                        const std::filesystem::path& userPresetDirectory)
  {
    ...
    {
      JsonStore& store;
      explicit BusyTimeoutGuard(JsonStore& s) : store(s) { store.SetBusyTimeoutMs(kMigrationBusyTimeoutMs); }
      ~BusyTimeoutGuard() { store.SetBusyTimeoutMs(JsonStore::kDefaultBusyTimeoutMs); }
