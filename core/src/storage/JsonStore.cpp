...
namespace guitarfx::storage
{
  namespace
  {
    ...
    public:
      ...
      ~Stmt() { sqlite3_finalize(mStmt); }

      Stmt(const Stmt&) = delete;
      Stmt& operator=(const Stmt&) = delete;

      [[nodiscard]] bool Ok() const { return mRc == SQLITE_OK && mStmt != nullptr; }
      [[nodiscard]] sqlite3_stmt* Get() const { return mStmt; }
      ...
      void BindInt64(int index, std::int64_t value) const { sqlite3_bind_int64(mStmt, index, value); }

      [[nodiscard]] int Step() const { return sqlite3_step(mStmt); }
      ...
      [[nodiscard]] std::int64_t ColumnInt64(int index) const { return sqlite3_column_int64(mStmt, index); }
  ...
  bool JsonStore::PutRawLocked(std::string_view type, std::string_view id, std::string_view json)
  {
    ...
    Stmt stmt(mDb,
              "INSERT INTO items(type, id, json, updated_at) VALUES(?1, ?2, ?3, ?4) "
