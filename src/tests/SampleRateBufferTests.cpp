...
void PrintStats(const std::string& label, const SignalStats& stats)
{
  std::cout << "    " << label << ": Peak=" << std::fixed << std::setprecision(4) << stats.peak 
            << ", RMS=" << stats.rms;
  if (stats.hasNaN) std::cout << " [NaN:" << stats.numNaN << "]";
  if (stats.hasInf) std::cout << " [Inf:" << stats.numInf << "]";
  if (stats.isAllZeros) std::cout << " [ZEROS]";
  std::cout << "\n";
}
...
bool TestSampleRates(const fs::path& modelPath)
{
  ...
  for (int i = 0; i < kNumSampleRates; ++i)
  {
    ...
    {
      std::cerr << "  ERROR: Failed to load model at " << sampleRate << " Hz\n";
      allPassed = false;
      continue;
    }
...
bool TestBufferSizes(const fs::path& modelPath)
{
  ...
  for (int i = 0; i < kNumBufferSizes; ++i)
  {
    ...
    if (!dsp.LoadModel(modelPath))
    {
      std::cerr << "  ERROR: Failed to load model with buffer size " << blockSize << "\n";
      allPassed = false;
      continue;
    }
...
bool TestDynamicBufferChanges(const fs::path& modelPath)
{
  ...
  dsp.Prepare(sampleRate, currentBlockSize);
  ...
  for (int i = 0; i < numChanges; ++i)
  {
    ...
    dsp.Prepare(sampleRate, newBlockSize);
...
bool TestSampleRateMismatch(const fs::path& modelPath)
{
  ...
  // Scenario 2: Process without calling Prepare first
  {
    ...
    if (!dsp.LoadModel(modelPath))
    {
      std::cout << "  Model loading without Prepare: ";
    ...
    try
    {
      ...
      if (stats.hasNaN || stats.hasInf)
      {
        std::cout << "  WARNING: Processing without Prepare() caused NaN/Inf\n";
        allPassed = false;
      }
      else if (stats.isAllZeros)
      {
        std::cout << "  Note: Output is silence (passthrough mode without model)\n";
      }
      else
      {
        std::cout << "  Processing worked (model may have default state)\n";
      }
    }
  ...
  // Scenario 3: Very high sample rate
  {
    ...
    if (!dsp.LoadModel(modelPath))
    {
      std::cerr << "  ERROR: Failed to load model at 192kHz\n";
    }
    else
    {
      dsp.SetInputTrim(0.0);
