...
// ============================================================================
// VENDORED FROM OBOE (shipped inside JUCE) — DO NOT EDIT CASUALLY.
//
// Source: juce/JUCE/modules/juce_audio_devices/native/oboe/include/oboe/
//         AudioStreamBase.h   (the Oboe copy bundled with JUCE 8.0.14)
//
// This copy is placed FIRST on the include path for the whole Android build by
// android/CMakeLists.txt, so every translation unit — Oboe's own sources and
// JUCE's alike — sees it instead of the original. Only one value differs: the
// default capture InputPreset.
//
// Why a header overlay rather than a call site: JUCE opens the Oboe stream in
// AndroidOboeAudioIODevice::open() (juce_Oboe_android.cpp), which constructs a
// local oboe::AudioStreamBuilder and never calls setInputPreset(). JUCE exposes
// no config flag for it, oboe::DefaultStreamValues covers only sample rate,
// burst size and channel count, and the file is inside the JUCE submodule so it
// cannot be forked in this repository. The default member initialiser in this
// header is the only lever.
//
...
// deliberately. Unprocessed is a request — AAudio substitutes another source on
// devices that do not implement it rather than failing to open — so the
// downside is a silent fallback rather than a broken stream. Doing it properly
// needs JUCE to expose the preset; see docs/android-build.md.
//
// The build guards against drift: android/app/build.gradle.kts pins the
// upstream file's SHA-256 and checkVendoredOboeHeader fails if it changes.
// ============================================================================
...
protected:
    ...
    */
    // ---- Soundshed patch --------------------------------------------------
    // Upstream default is InputPreset::VoiceRecognition. That routes capture
    // through the device's voice path, which on Samsung hardware in particular
    // means AGC, noise suppression and band-limiting — everything you do not
    // want in front of an amp sim, plus added latency.
    //
    // Unprocessed is the preset Android defines for instrument-style capture.
    // See android/CMakeLists.txt for why this is patched here rather than at
    // the call site.
    InputPreset                     mInputPreset = InputPreset::Unprocessed;
    // -----------------------------------------------------------------------
